...
@triton.jit
def _fused_gather_dequant_model1_inner(
    KV_Cache, Indices, TopkLength,
    OutputKV, OutputMask,
    total_tokens, topk, block_size, k_offset, s_q,
    stride_kv_block,
    stride_idx_t, stride_idx_k,
    stride_out_t, stride_out_k, stride_out_d,
    stride_mask_t, stride_mask_k,
    pid, BLOCK_TK: tl.constexpr,
    ...
    """Inner function for processing one scope."""
    num_tk = total_tokens * topk
    offs_tk = pid * BLOCK_TK + tl.arange(0, BLOCK_TK)
    mask_tk = offs_tk < num_tk
    t_idx = offs_tk // topk
...
def fused_gather_dequant_fp8_model1(
    ...
    """Fused MODEL1 gather - optimized wrapper for both scopes."""
